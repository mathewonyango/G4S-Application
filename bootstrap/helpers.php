...
if (!function_exists('curl')) {
    function curl($data)
    {
       $url = 'http://18.198.168.12:9977/channelinterface/g4s';
        //$url = 'http://127.0.0.1:8770/channelinterface/courier';
		//$url = 'http://127.0.0.1:9977/channelinterface/g4s';
        $ch = curl_init($url);
...
function makerCheckerName($user_id)
{
    ...
    if ($user_id == 777 || $user_id == 0 || $user_id == '') {
        ...
        if ($user_id == '') {
            ...
            if (isset(User::find($user_id)->username)) {
                $maker = (User::find($user_id)->username);
            } else {
                $maker = "G4S Admin";
            }
            if (isset(User::find($user_id)->username)) {
...
if (!function_exists('getRegionName')) {
    ...
    function getRegionName($id)
    {
        ...
        $from = $region[0]->town_name;
        return $from;
    }
}
if (!function_exists('getRegionName2')) {

    function getRegionName2($id)
    {
        $region = DB::table('regions')->where('id', $id)->get();
        $to= $region[0]->town_name;
        return $to;
    }
}


if (!function_exists('getRiderType')) {

    function getRiderType($id)
    {
        $riders = DB::table('roles')->where('id', $id)->get();
        $type = $riders[0]->name; 
        $rider = $type;
        return $rider;
    }
    ...
    function variance($id)
{
    ...
    if ($shipment) {
        ...
        $dateDelivered = $shipment->date_delivered ?  Carbon::parse($shipment->date_delivered) : Carbon::now();
        
        $daysDifference = $dateDelivered->diffInDays($createdAt);
        
        $modifiedDifference = $daysDifference; // Subtract 2 days
...
if (!function_exists('getRiderName')) {
    ...
    function getRiderName($id)
    {
        ...
        if ($id == '') {
            ...
            $firstname = $riders[0]->firstname; 
            $lastname = $riders[0]->lastname;
            // dd($maker);
            if(is_null($riders)){
               $riders = 'Not assigned';
            }else{
                $riders = $firstname . " " . $lastname;
            }
        }
...
if (!function_exists('getMakerName')) {
    ...
    function getMakerName($id)
    {
        ...
        $maker = DB::table('users')->where('id', $id)->get();
        $firstname = $maker[0]->firstname; 
        $lastname = $maker[0]->lastname;
            // dd($maker);
            if(is_null($maker)){
               $maker = 'Not assigned';
            }else{
                $maker = $firstname . " " . $lastname;
            }
        }
        return $maker;
    }
...
{
    $url = 'https://maps.googleapis.com/maps/api/geocode/json?latlng='.$lat.','.$lng.'&sensor=false&key=AIzaSyCbwg43kyvUb6GUikh17Cy_8sGFBEkJEHs';
...
if (!function_exists('status_label')) {
    ...
    function status_label($status)
    {
        ...
        switch ($status) {
            ...
            case in_array($status, ['active', 'published', 'paid', 'complete', 'confirmed', 'valid', 'live', 'completed', 'resolved']):
...
function getCorporateName($corporate_id)
{
    $returnCorporateName = \App\Corporate::all()->where('corporate_id', '=', $corporate_id)->first();
    return $returnCorporateName->name;
}

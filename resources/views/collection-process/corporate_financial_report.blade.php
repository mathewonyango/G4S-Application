...
<div class="col-sm-3">
<form action="{{ route('report.post-corporate-financial-report') }}" method="get">

    
                <div class="form-group">
                    <label for="branch"> Sort report by Corporates:</label>
                    <select name="corporate" id="corporate" class="form-control">
                        @foreach($corporates as $corporate)
                        <option value="{{ $corporate->corporate_id }}">{{ $corporate->name }}</option>
                        @endforeach
                    </select>
                </div>
